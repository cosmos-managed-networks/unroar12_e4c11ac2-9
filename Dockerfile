...
# then log output of file /code/bin/unroar12
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/unroar12 \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/unroar12 | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/unroar12 /usr/bin/unroar12
