...
HTTPS_GIT := github.com/unroar12/unroar12.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=unroar12 \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=unroar12 \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/unroar12 ./cmd/unroar12
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/unroar12.exe ./cmd/unroar12
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/unroar12
...
draw-deps:
	...
	@goviz -i ./cmd/unroar12 -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c unroar12 --local -f chains.yaml
endif
...
set-testnet-configs:
	unroar12 config set client chain-id chainid-1
	unroar12 config set client keyring-backend test
	unroar12 config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | unroar12 keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | unroar12 keys add acc1 --recover`
